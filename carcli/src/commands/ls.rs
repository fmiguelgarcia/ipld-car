use crate::commands::common::{fmt_size, pick_icon};
...
impl SubCmdLs {
	pub fn run(&self) -> Result<()> {
		...
		if self.tree {
			...
			for name in &entries {
				let (file_type, size_str, icon, suffix) = entry_info(&fs, &self.path, name)?;
...
fn entry_info(fs: &CarFs<File>, parent: &str, name: &str) -> Result<(VfsFileType, String, char, &'static str)> {
	...
	match fs.metadata(&child_path) {
		Ok(meta) => {
			let size_str = match meta.file_type {
				...
				VfsFileType::File => fmt_size(meta.len),
			};
...
fn print_tree(fs: &CarFs<File>, path: &str, prefix: &str) -> Result<()> {
	let entries: Vec<String> = fs.read_dir(path)?.collect();
	for (i, name) in entries.iter().enumerate() {
		let last = i == entries.len() - 1;
		let connector = if last { "└── " } else { "├── " };
		let (file_type, _, icon, suffix) = entry_info(fs, path, name)?;
		...
		if file_type == VfsFileType::Directory {
			...
			print_tree(fs, &child_path, &format!("{prefix}{extension}"))?;
		}
	}
	Ok(())
}
